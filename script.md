...
This final dynamical slide ties the bifurcation diagram to a real brain-network
simulation. The Generic2dOscillator runs on the Desikan-Killiany connectome via
TVBO and the tvboptim backend. The animation shows three synchronized views.
The two left panels are the Desikan-Killiany regions projected onto axial (top)
and sagittal views: each circle is a brain region, sized by structural
in-strength and colored by its instantaneous membrane potential $V(t)$. The
right panel is the bifurcation diagram in $I$. Each node is also drawn there at
its current effective input and current voltage $(I_{\mathrm{eff},i}, V_i(t))$,
using the same color scale.

The point is that a network simulation is not just a collection of time series.
At every moment, each node occupies a position in the regime map. Nodes with
low effective input sit near resting fixed-point regions and stay near zero on
the brain views. Nodes whose effective input crosses the oscillatory window
move into the part of the diagram where periodic orbits exist, and the
corresponding region on the brain lights up red and blue as it oscillates.
Coupling makes the nodes move through this map together. The last part of the
animation holds the drive fixed so the audience can see where the network
states settle. The bifurcation diagram becomes a coordinate system for reading
the network dynamics, and the brain panels show that this coordinate system
maps onto anatomy.

The speaker line is: the bifurcation diagram is the atlas, and the brain
network simulation is a moving path through that atlas.