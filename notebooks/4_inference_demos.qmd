...
## Parallelism — grid in, losses out

Two-panel teaser for the parallelism slide. Schematic on a 10×10 grid
over abstract $(\theta_1, \theta_2)$ with a synthetic Gaussian loss —
the point is the *shape of the API* (grid in, losses out), not the
specific running example.
...
## Method ceiling — where sampling stops scaling

Horizontal bars: each method's usable range in parameter dimension $d$.
The shaded band marks the regional-parameter regime ($d \sim 10^3$–$10^4$)
that brain network models live in. Only gradient descent crosses it.
...
## Prior pictograms — uniform / weakly / strongly informative

Tiny inline pictograms used as bullet decorations on the priors slide.
Each is a square, axes-off, single-color sketch of one prior shape over
the same range.
...
## Bayesian shrinkage cartoon — prior, likelihood, posterior

A generic textbook visual to introduce Bayes on the slide: a wide prior
times a peaked likelihood gives a tighter posterior. No connection to
the running example.
...
## Pipeline overview cartoon

A 4-panel cartoon for the "default tvboptim recipe" slide: bifurcation
diagram → coarse-search loss landscape → gradient descent (global → regional)
→ Bayesian posterior. All synthetic, all cividis.
