...
```{python}
fig = sweep.plot(overlay=True)
fig.suptitle("Sweep of Generic2dOscillator parameter a")
plt.show()
```
...
result = exp.run("tvboptim")
fig = result.explorations["trials"].plot(overlay=True)
fig.suptitle("Ten trials with randomised initial conditions")
plt.show()
```
