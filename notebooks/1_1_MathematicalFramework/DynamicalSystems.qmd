...
For a single local system, `Dynamics.run()` is the shortest route from equations to a simulated trajectory.
...
TVBO objects also know how to plot themselves. A quick state-variable plot does not need custom notebook helper functions.
...
For the workshop workflow we usually wrap the model in a `SimulationExperiment`. The model still describes the equations; the experiment describes how to run them.
